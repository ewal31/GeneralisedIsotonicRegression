...
static void BM_points_to_adjacency_dominated_sorting_2d(benchmark::State& state) {
    for (auto _ : state) {
        state.PauseTiming();
        const auto [points, y] = gir::generate_monotonic_points(state.range(0), 0.1, 2);
        state.ResumeTiming();
        benchmark::DoNotOptimize(gir::points_to_adjacency_dominated_sorting(points));
        state.SetComplexityN(state.range(0));
    }
}
BENCHMARK(BM_points_to_adjacency_dominated_sorting_2d)
    ->RangeMultiplier(2)->Range(1<<10, 1<<17)->Complexity();
